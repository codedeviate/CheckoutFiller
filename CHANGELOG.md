...
### Fixed
- Corrected the B2B (company) org numbers to each provider's documented test value
  (the 0.6.0 default `556001-1982` is Qliro-specific and isn't recognized by the
  others): **Svea → `4608142222`** (Svea's B2B test "NationalId" is a 10-digit
  numeric string, not the 556-format), **Kustom → `556131-2058`** (docs.kustom.co).
  Qliro keeps `556001-1982`. All three pass the org-number checksum.
...
### Fixed
- Recognize `orgno` / `orgnr` / `org number` / `organisationsnummer` field names
  (→ `ssn`). The NEH platform names its personal-number field `orgno` because the
  same field is used for company org numbers; the matcher now fills it.

## [0.5.0] - 2026-06-04

### Added
- **Opt-in auto-refill** (off by default; toggle on the options page). After you
  trigger a fill, the extension keeps filling newly-revealed recognized fields in
  the same frame from the same selection for a sliding 15s window — so step-by-step
  checkouts like Qliro fill through their steps from one action. Only fills empty
  fields, same frame only, and self-disarms after 15s idle or on navigation.
...
### Added
- Extension and toolbar icon (16/32/48/128 px) — a green rounded square with a
  white checkmark, matching the badge colour. Generated from
  `tools/make-icons.js` and copied into both builds.
...
[Unreleased]: https://github.com/codedeviate/CheckoutFiller/compare/v0.6.1...HEAD
[0.6.1]: https://github.com/codedeviate/CheckoutFiller/compare/v0.6.0...v0.6.1
